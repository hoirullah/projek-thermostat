...
void Fan_Control(double temp)
{
	if (temp > 39.0)
	{
		  PORTD &= ~(1<<PD7);/* Turn on the fan motor */
	}
	else if (temp < 38.5)
	{
		 PORTD |= (1<<PD7);  /* Turn off the fan motor */
	}
}
...
int main(void)
{
	...
	DDRD |= (1<<PD7);  /* Set PD7 as output for fan control */
	PORTD &= ~(1<<PD7);  /* Initialize fan motor off */
	...
	while (1)
	{
		...
		Fan_Control(temp);  /* Control the fan motor based on temperature */
		
		_delay_ms(1500);  /* wait for 1 second */
	}
}
